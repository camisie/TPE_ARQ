...
void dateToString(date myDate, char *buffer, char separator, int length) {
    itoa1(myDate->day, buffer, 2);
    buffer[2] = separator;
    itoa1(myDate->month, buffer + 3, 2);
    buffer[5] = separator;
    itoa1(myDate->year, buffer + 6, 2);
    buffer[8] = ' ';
    itoa1(myDate->hour, buffer + 9, 2);
    buffer[11] = ':';
    itoa1(myDate->minutes, buffer + 12, 2);
    buffer[14] = ':';
    itoa1(myDate->seconds, buffer + 15, 2);
    buffer[18] = 0;
}

void timeToString(date myDate, char *buffer, char separator, int length) {
    itoa1(myDate->hour, buffer, 2);
    buffer[2] = ':';
    itoa1(myDate->minutes, buffer + 3, 2);
    buffer[5] = ':';
    itoa1(myDate->seconds, buffer + 6, 2);
    buffer[9] = 0;
}
